...
class UpdateFlightViewModel extends BaseViewModel {
  ...
  UpdateFlightViewModel({required this.flight}) {
    _selectedDepartureTime = flight.departureTime;
    _selectedArrivalTime = flight.arrivalTime;
    _selectedStatus = flight.status;
    gateController.text = flight.gate ?? '';
    terminalController.text = flight.terminal ?? '';
    airplaneIdController.text = flight.airplaneId ?? '';
  }
  ...
  Future<void> updateFlight() async {
    ...
    try {
      ...
      await _flightService.updateFlight(flight.id, flightUpdate);
